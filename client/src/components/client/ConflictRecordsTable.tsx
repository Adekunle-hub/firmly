...
export default function ConflictingRecordsTable({ conflicts }: { conflicts: ConflictItem[] }) {
  ...
  return (
    <section className="bg-white border border-slate-200/80 rounded-2xl p-6 shadow-sm flex flex-col gap-4">
      <h2 className="text-sm font-bold text-slate-500 uppercase tracking-wider">Conflicting Records</h2>
      ...
      <div className="flex flex-col gap-1.5">
        <label className="text-sm font-semibold text-slate-500">Related Cases</label>
        <div className="border border-slate-100 rounded-xl bg-white divide-y divide-slate-50 overflow-hidden">
          {caseConflicts.map((c, i) => (
            <div key={i} className="flex items-center gap-2 px-4 py-3 text-sm text-slate-600 hover:bg-slate-50 cursor-pointer transition-colors">
              <FileDown className="w-3.5 h-3.5 text-slate-400" />
              {c.name}
            </div>
          ))}
          {caseConflicts.length === 0 && (
            <div className="px-4 py-3 text-sm text-slate-400">No related cases found.</div>
          )}
        </div>
      </div>
    </section>
